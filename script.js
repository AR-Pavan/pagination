...
//creating div element and adding id container 
let divElement = document.createElement("div");
document.body.appendChild(divElement);
let div = document.getElementsByTagName("div")[0];
let idAttr = document.createAttribute("id");
idAttr.value = "container";
div.setAttributeNode(idAttr);
//creating table element and appending it to div element
let tableElement = document.createElement("table");
div.appendChild(tableElement);
let tableClass = document.createAttribute("class");
tableClass.value = "table table-hover";
...
//creating second div and appending it to the first div
let div2 = document.createElement("div");
div.appendChild(div2);
let classAttr = document.createAttribute("class");
classAttr.value = "pagination";
div2.setAttributeNode(classAttr);
let id2 = document.createAttribute("id");
id2.value = "Pagination";
div2.setAttributeNode(id2);

//To display the navigation page
let displayPageNav = perPage => {
    let pagination = ``;
    ...
    for (let i = 1; i <= pages; i++) {
        pagination += `<button class="page btn btn-sm btn-info" onClick="displayItems(${i},${perPage})">${i} </button>`
    }
...
//function to display the json data in table 
let displayItems = (page, perPage) => {
    let index, offset
    if (page == 1) {
        index = 0;
        offset = perPage;
    ...
    let slicedItems = dataSet.slice(index, offset);
    let html = slicedItems.map(item =>
        `<tr>
        <td>${item.id}</td>
        <td>${item.name}</td>
        <td>${item.email}</td>
